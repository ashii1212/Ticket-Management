...
export const AdminTicketList: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['adminTickets', page, searchTerm, statusFilter, priorityFilter, slaStatusFilter, assigneeFilter],
    queryFn: () => getTickets({ page, search: searchTerm, status: statusFilter, priority: priorityFilter, slaStatus: slaStatusFilter, assignee: assigneeFilter, limit: 15 })
  });
  ...
  const filters = (
    <div className="flex flex-wrap gap-2">
      ...
      <input 
        ...
        className="border border-slate-300 rounded-md text-sm px-2 py-1.5 bg-white w-36" 
      />
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-slate-800">All Tickets (Admin)</h1>
