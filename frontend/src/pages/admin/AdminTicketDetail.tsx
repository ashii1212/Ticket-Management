...
import { Clock, MessageSquare, Activity as ActivityIcon, Users } from 'lucide-react';
...
export const AdminTicketDetail: React.FC = () => {
  ...
  // Modals state
  const [activeModal, setActiveModal] = useState<'NONE' | 'PENDING' | 'RESOLVE' | 'ESCALATE' | 'REASSIGN'>('NONE');
  ...
  const actionMutation = useMutation({
    mutationFn: async ({ action, payload }: { action: string, payload?: any }) => {
      switch (action) {
        case 'ASSIGN': return assignTicket(ticketId, payload || user!.id);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <button onClick={() => setActiveModal('REASSIGN')} disabled={isPending} className="px-4 py-2 bg-slate-200 text-slate-800 rounded-md text-sm font-medium hover:bg-slate-300 disabled:opacity-50 flex items-center gap-2">
            <Users className="w-4 h-4"/> Reassign
          </button>
          <div className="w-px h-6 bg-slate-300 mx-2"></div>
          {ticket.status === 'ASSIGNED' && (
            <button onClick={() => actionMutation.mutate({ action: 'START' })} disabled={isPending} className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">Force Start</button>
          )}
          {ticket.status === 'IN_PROGRESS' && (
            <>
              <button onClick={() => setActiveModal('PENDING')} disabled={isPending} className="px-4 py-2 bg-amber-500 text-white rounded-md text-sm font-medium hover:bg-amber-600 disabled:opacity-50">Force Request Info</button>
              <button onClick={() => setActiveModal('ESCALATE')} disabled={isPending} className="px-4 py-2 bg-red-600 text-white rounded-md text-sm font-medium hover:bg-red-700 disabled:opacity-50">Force Escalate</button>
              <button onClick={() => setActiveModal('RESOLVE')} disabled={isPending} className="px-4 py-2 bg-green-600 text-white rounded-md text-sm font-medium hover:bg-green-700 disabled:opacity-50">Force Resolve</button>
            </>
          )}
          {ticket.status === 'PENDING_STUDENT' && (
            <button onClick={() => actionMutation.mutate({ action: 'RESUME' })} disabled={isPending} className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">Force Resume Work</button>
          )}
          {ticket.status === 'RESOLVED' && (
             <button onClick={() => actionMutation.mutate({ action: 'REOPEN' })} disabled={isPending} className="px-4 py-2 bg-slate-600 text-white rounded-md text-sm font-medium hover:bg-slate-700 disabled:opacity-50">Force Reopen</button>
          )}
        </div>
      </div>
      ...
      <Modal isOpen={activeModal !== 'NONE'} onClose={() => setActiveModal('NONE')} title={activeModal === 'REASSIGN' ? 'Reassign Ticket' : activeModal === 'PENDING' ? 'Request Info' : activeModal === 'RESOLVE' ? 'Resolve Ticket' : 'Escalate Ticket'}>
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            {activeModal === 'REASSIGN' && "Enter the staff ID to assign this ticket to."}
          ...
          <div className="flex justify-end gap-3">
            ...
            <button 
              onClick={() => actionMutation.mutate({ action: activeModal, payload: activeModal === 'REASSIGN' ? Number(reassignInput) : modalText })}
              disabled={(activeModal === 'REASSIGN' ? !reassignInput : modalText.trim().length === 0) || isPending}
